...
4. `data/progress-baseline.json`
5. `web-build/index.html`
...
3. Identify the next open task from the 82-step roadmap.
4. Implement only the smallest coherent slice needed for that task.
5. Keep branch boundaries explicit.
6. Update `docs/PROJECT_STATE.md` with date, status, evidence, and blockers.
7. Update `data/progress-baseline.json` when statuses change.
8. Mirror public docs/data into `web-build/docs/` and `web-build/data/`.
9. Validate the static site and any new code.
10. Deploy `web-build/` to the server and push the commit to GitHub.
...
- Verify the hosted blueprint PDF and progress JSON return HTTP 200.
